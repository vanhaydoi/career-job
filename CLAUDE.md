...
**Career-ops is designed to be made yours.** If the archetypes don't match your career, the modes are in the wrong language, or the scoring doesn't fit your priorities -- just ask. You (AI Agent) can edit the user's files. The user says "change the archetypes to data engineering roles" and you do it. That's the whole point.
...
>
> Tip: Having a personal portfolio dramatically improves your job search. Showcase your projects, case studies, and achievements to stand out from other candidates."
...
- **Vietnamese (Vietnam market):** `modes/vi/` — native Vietnamese translations with Vietnam-specific vocabulary (hợp đồng lao động, thử việc, BHXH/BHYT/BHTN, lương gross/net, lương tháng 13, thưởng Tết, phép năm, ITviec, VietnamWorks, TopCV, etc.). Includes `_shared.md`, `danh-gia.md` (evaluation), `ung-tuyen.md` (apply), `pipeline.md`.

**When to use Vietnamese modes:** If the user is targeting Vietnamese-language job postings, lives in Vietnam, or asks for Vietnamese output. Either:
1. User says "use Vietnamese modes" or "dùng mode tiếng Việt" → read from `modes/vi/` instead of `modes/`
2. User sets `language.modes_dir: modes/vi` in `config/profile.yml` → always use Vietnamese modes
3. You detect a Vietnamese JD (from ITviec, VietnamWorks, TopCV) → suggest switching to Vietnamese modes

**When NOT to:** If the user applies to English-language roles, even at Vietnamese companies, use the default English modes.
